...
export type RoundButtonDisplay = 'default' | 'outline' | 'inverted';
const displays: { [key in RoundButtonDisplay]: {
    ...
    default: {
        ...
        borderPressedColor: Theme.accentPressed,
        textPressed: '#fff',
    },
    outline: {
        ...
        >
            {(p) => (
                <View style={{ height: size.height - 2, alignItems: 'center', justifyContent: 'center', minWidth: 64, paddingHorizontal: 16, }}>
                    ...
                    <Text style={[{ marginTop: size.pad, opacity: doLoading ? 0 : 1, color: p.pressed ? display.textPressed : display.textColor, fontSize: size.fontSize, fontWeight: '600', includeFontPadding: false }]} numberOfLines={1}>{props.title}</Text>
                </View>
            )}
        </Pressable>
    )
